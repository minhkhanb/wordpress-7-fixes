	...
	private function define_constants() {
		...
		if ( unicpo_fs()->is__premium_only() ) {
			if ( 'local' === $plugin_settings['file_storage'] ) {
				if ( ! empty( $plugin_settings['custom_path_enable'] ) && ! empty( $plugin_settings['custom_path'] ) ) {
					$path = trim( $plugin_settings['custom_path'], '/' );
					$this->define( 'UNI_CPO_FILES_DIR', wp_normalize_path( trailingslashit( $upload_dir['basedir'] ) . $path ) );
					$this->define( 'UNI_CPO_FILES_URI', trailingslashit( $upload_dir['baseurl'] ) . trailingslashit( $path ) );
				} else {
					$this->define( 'UNI_CPO_FILES_DIR', wp_normalize_path( trailingslashit( $upload_dir['basedir'] ) . 'cpo-uploads' ) );
					$this->define( 'UNI_CPO_FILES_URI', trailingslashit( $upload_dir['baseurl'] ) . trailingslashit( 'cpo-uploads' ) );
				}
			}
		}
	}
	...
	function admin_scripts() {
		...
		if ( in_array( $screen_id, array( 'product', 'edit-product' ) ) ) {
			...
			wp_register_script(
				...
				array( 'wc-admin-order-meta-boxes', 'moment', 'flatpickr', 'parsleyjs' ),
				UNI_CPO_VERSION
			);
			...
			$uni_cpo_i18n = apply_filters( 'uni_cpo_i18n_admin_strings',
				array(
		...
		} elseif ( in_array( $screen_id, array(
			'woocommerce_page_uni-cpo-import-export',
			'woocommerce_page_uni-cpo-settings'
		) ) ) {
			wp_enqueue_style(
			...
			'woocommerce_page_uni-cpo-manager'
		) ) ) {
			wp_register_script(
				'uni-cpo-manager',
