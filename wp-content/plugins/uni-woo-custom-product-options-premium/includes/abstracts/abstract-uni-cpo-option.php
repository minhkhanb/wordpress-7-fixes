...
class Uni_Cpo_Option extends Uni_Cpo_Data {
	...
	*/
	...
	public function cpo_order_label() {
		...
		$cpo_order_label = ( ! empty( $cpo_general['advanced']['cpo_order_label'] ) )
			? $cpo_general['advanced']['cpo_order_label']
			: 0;
		if ( ! empty( $cpo_order_label ) ) {
			return $cpo_order_label;
		} else {
			return ( ! empty( $cpo_general['advanced']['cpo_label'] ) )
				? $cpo_general['advanced']['cpo_label']
				: $this->get_slug();
		}
	}
	...
	public function cpo_order_visibility() {
		$cpo_general     = $this->get_cpo_general();
		// hide?
		return ( ! empty( $cpo_general['advanced']['cpo_order_visibility'] ) && 'yes' === $cpo_general['advanced']['cpo_order_visibility'] )
			? true
			: false;
	}
			...
			<script>
                jQuery(document).ready(function($) {
                    ...
                    $(document.body).on('uni_cpo_options_data_ajax_success', function() {
						<?php echo esc_attr( $slug ) ?>_fields_conditional_func(unicpo.formatted_vars);
                    });
                    $(document.body).on('uni_cpo_options_data_for_conditional', function(e, fields) {
                        var variables = $.extend({}, unicpo.formatted_vars, fields);
						<?php echo esc_attr( $slug ) ?>_fields_conditional_func(variables);
                    });

                    function <?php echo esc_attr( $slug ) ?>_fields_conditional_func(formData) {
                        try {
                            var $<?php echo esc_attr( $slug ) ?>          = $('#<?php echo esc_attr( $slug ) ?>');
                            var $<?php echo esc_attr( $slug ) ?>_fields = $<?php echo esc_attr( $slug ) ?>.find('input, select, textarea');
	...
	public static function suboptions_conditional_rules( $data, $attributes = array() ) {
		...
		if ( unicpo_fs()->is__premium_only() ) {
			$rules_data = ( isset( $data['settings']['cpo_rules']['main'] ) )
				? $data['settings']['cpo_rules']['main']
				: array();

			$is_enabled = ( isset( $rules_data['cpo_is_sc'] ) && 'yes' === $rules_data['cpo_is_sc'] ) ? true : false;
			$sc_default = ( isset( $rules_data['cpo_sc_default'] ) ) ? $rules_data['cpo_sc_default'] : 'hide';

			if ( ! $is_enabled ) {
				return;
			}

			$rules = ( isset( $rules_data['cpo_sc_scheme'] ) ) ? $rules_data['cpo_sc_scheme'] : array();
			$slug  = UniCpo()->get_var_slug() . $data['settings']['cpo_general']['main']['cpo_slug'];
			$type  = $data['type'];
			...
			if ( is_array( $rules ) && ! empty( $rules ) ) {
				...
				$main_code       = '';
				$else_code       = '';
				$final_statement_first = '';
				$final_statement_last = '';

				if ( 'select' === $type ) {
                    $final_statement_first = 'var $' . $slug . ' = $("#' . $slug . '-field");';
                    $final_statement_last = '$' . $slug . '.parsley().validate();';

					$main_code = 'var selectOptions = document.getElementById("' . $slug . '-field").options;' . "\n";
					$main_code .= 'for (var i = 0; i < selectOptions.length; i++) {' . "\n";
					$main_code .= 'if (optionsFromRule.includes(selectOptions[i].value)) {' . "\n";
					if ( 'hide' === $sc_default ) {
						$main_code .= 'selectOptions[i].disabled = false;' . "\n";
						$main_code .= 'selectOptions[i].hidden = false;' . "\n";
					} else {
						$main_code .= 'selectOptions[i].disabled = true;' . "\n";
						$main_code .= 'selectOptions[i].hidden = true;' . "\n";
					}
					$main_code .= '} else {' . "\n";
					if ( 'hide' === $sc_default ) {
						$main_code .= 'selectOptions[i].disabled = true;' . "\n";
					...
					$else_code = 'var selectOptions = document.getElementById("' . $slug . '-field").options;' . "\n";
					if ( 'hide' === $sc_default ) {
						$else_code .= 'for (var i = 0; i < selectOptions.length; i++) {' . "\n";
						$else_code .= 'selectOptions[i].disabled = true;' . "\n";
						$else_code .= 'selectOptions[i].hidden = true;' . "\n";
						$else_code .= '}' . "\n";
					} else {
						$else_code .= 'for (var i = 0; i < selectOptions.length; i++) {' . "\n";
						$else_code .= 'selectOptions[i].disabled = false;' . "\n";
						$else_code .= 'selectOptions[i].hidden = false;' . "\n";
						$else_code .= '}' . "\n";
					}
				}

                if ( 'radio' === $type || 'checkbox' === $type ) {
                    //$final_statement_first = '/*var $' . $slug . ' = $("input[name=\"'. $slug .'\"]");*/';
                    //$final_statement_last = '/*$' . $slug . '.parsley();*/';

                    if ( 'radio' === $type ) {
                        $main_code = 'var radioOptions = document.getElementsByName("' . $slug . '");' . "\n";
                    }
                    if ( 'checkbox' === $type ) {
                        $main_code = 'var radioOptions = document.getElementsByClassName("' . $slug . '-field");' . "\n";
                    }

                    $main_code .= 'for (var i = 0; i < radioOptions.length; i++) {' . "\n";
                    $main_code .= 'if (optionsFromRule.includes(radioOptions[i].value)) {' . "\n";

                    if ( 'hide' === $sc_default ) {
                        $main_code .= 'radioOptions[i].disabled = false;' . "\n";
                        $main_code .= 'radioOptions[i].hidden = false;' . "\n";
                    } else {
                        $main_code .= 'radioOptions[i].disabled = true;' . "\n";
                        $main_code .= 'radioOptions[i].hidden = true;' . "\n";
                        $main_code .= 'radioOptions[i].checked = false;' . "\n";
                    }
                    $main_code .= '} else {' . "\n";
                    if ( 'hide' === $sc_default ) {
                        $main_code .= 'radioOptions[i].disabled = true;' . "\n";
                    ...
                    if ( 'radio' === $type ) {
                        $else_code = 'var radioOptions = document.getElementsByName("' . $slug . '");' . "\n";
                    }
                    if ( 'checkbox' === $type ) {
                        $else_code = 'var radioOptions = document.getElementsByClassName("' . $slug . '-field");' . "\n";
                    }

                    if ( 'hide' === $sc_default ) {
                        $else_code .= 'for (var i = 0; i < radioOptions.length; i++) {' . "\n";
                        $else_code .= 'radioOptions[i].disabled = true;' . "\n";
                        $else_code .= 'radioOptions[i].hidden = true;' . "\n";
                        $else_code .= 'radioOptions[i].checked = false;' . "\n";
                        $else_code .= '}' . "\n";
                    } else {
                        $else_code .= 'for (var i = 0; i < radioOptions.length; i++) {' . "\n";
                        $else_code .= 'radioOptions[i].disabled = false;' . "\n";
                        $else_code .= 'radioOptions[i].hidden = false;' . "\n";
                        $else_code .= '}' . "\n";
                    }
                }
				...
				foreach ( $rules as $rule ) {
					...
					$suboptions      = $rule['options'];
					$suboptions_json = json_encode( $suboptions );

					$condition = uni_cpo_option_js_condition_prepare( $scheme );

					if ( $i === 1 ) {
						$final_statement .= 'if ' . $condition . ' {' . "\n";
						$final_statement .= 'var optionsFromRule = ' . $suboptions_json . ';' . "\n";
						$final_statement .= $main_code;
						$final_statement .= '}';
					} elseif ( $i <= $count ) {
                ...
                if ( 'select' === $type ) {
                    $final_statement .= ' else {' . "\n";
                    $final_statement .= $else_code;
                    $final_statement .= '}' . "\n";
                    $final_statement .= 'selectOptions[0].hidden = false;' . "\n";
                    $final_statement .= 'var selectOption = document.getElementById("' . $slug . '-field");' . "\n";
                    $final_statement .= 'if (typeof selectOption.selectedOptions[0] !== "undefined" && selectOption.selectedOptions[0].disabled) {' . "\n";
                    $final_statement .= 'selectOption.value = "";' . "\n";
                    $final_statement .= '}' . "\n";
                }
				...
				<script>
                    jQuery(document).ready(function($) {
                        ...
                        $(document.body).on('uni_cpo_options_data_ajax_success', function() {
							<?php echo esc_attr( $slug ) ?>_suboptions_conditional_func(unicpo.formatted_vars);
                        });
                        $(document.body).on('uni_cpo_options_data_for_conditional', function(e, fields) {
							<?php echo esc_attr( $slug ) ?>_suboptions_conditional_func(fields);
                        });

                        function <?php echo esc_attr( $slug ) ?>_suboptions_conditional_func (formData) {
                            try {
                                <?php echo $final_statement_first; ?>

								<?php echo $final_statement; ?>

                                <?php echo $final_statement_last; ?>
                            } catch (e) {
                                console.error(e);
                            }
                        }
	...
	public static function validation_rules( $data, $attributes = array() ) {
		...
		if ( unicpo_fs()->is__premium_only() ) {
			...
			$rules = ( isset( $rules_data['cpo_vc_scheme'] ) ) ? $rules_data['cpo_vc_scheme'] : array();
			$slug  = UniCpo()->get_var_slug() . $data['settings']['cpo_general']['main']['cpo_slug'];
					...
					if ( $i === 1 ) {
						...
						if ( ! empty( $scheme_attrs ) ) {
							foreach ( $scheme_attrs as $k => $v ) {
								if ( ! empty ( $v ) ) {
									if ( false !== strpos( $v, 'formData' ) ) {
										$final_statement .= '$' . $slug . '.attr("' . esc_attr( $k ) . '", ' . esc_attr( $v ) . ');' . "\n";
										$final_statement .= '$' . $slug . '.siblings("input").attr("' . esc_attr( $k ) . '", ' . esc_attr( $v ) . ');' . "\n";
									} else {
										$final_statement .= '$' . $slug . '.attr("' . esc_attr( $k ) . '", "' . esc_attr( $v ) . '");' . "\n";
										$final_statement .= '$' . $slug . '.siblings("input").attr("' . esc_attr( $k ) . '", "' . esc_attr( $v ) . '");' . "\n";
									}
									...
									$final_statement .= '$' . $slug . '.siblings("input").removeAttr("' . esc_attr( $k ) . '");' . "\n";
								}
							}
						}
						$final_statement .= '}';
				...
				<script>
                    jQuery(document).ready(function($) {
                        ...
                        function <?php echo esc_attr( $slug ) ?>_validation_func (formData) {
                            try {
                                var $<?php echo esc_attr( $slug ) ?> = $('#<?php echo esc_attr( $slug ) ?>-field');
