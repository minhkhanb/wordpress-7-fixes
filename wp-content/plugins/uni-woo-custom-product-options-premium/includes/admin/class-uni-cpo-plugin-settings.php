...
class Uni_Cpo_Plugin_Settings {
	...
	private function settings_fields() {
		...
		$all_roles  = wp_roles();
		$role_names = array_merge( $all_roles->role_names, [ '' => __( 'Guest (unregistered)', 'uni-cpo' ) ] );
	...
	public function manager_page() {
		...
		if ( unicpo_fs()->is__premium_only() ) {
			$html .= '<p>' . esc_html__( 'Make changes, click "Save Data" when your ready to update the information or click "Reset Data" to cancel all changes and revert back to initial (last saved).', 'uni-cpo' ) . '</p>';
			$html .= '<p>' . esc_html__( 'FYI: if you see variables highlighted in red, it is a signal that something is wrong! Such variables are used in product configuration, but do not exist in DB. Which means they were overwritten. Immediately check those products, probably their price calculation are not functioning properly!', 'uni-cpo' ) . '</p>';
			$html .= '<div id="uniCpoManagerProducts"></div>';
		} else {
	...
	public function import_export_page() {
		...
		if ( unicpo_fs()->is__premium_only() ) {
			...
			foreach ( $types as $type ) {
				$options = $options + uni_cpo_get_similar_modules( array(
						'obj_type' => 'option',
						'type'     => $type,
						'pid'      => 0
					) );
			}
			$html .= '<label>' . esc_html__( 'Existing options', 'uni-cpo' ) . '</label>';
			$html .= '<select id="cpo_settings_option_id">';
			$html .= '<option value="0" disabled selected>- ' . esc_html__( 'select', 'uni-cpo' ) . ' -</option>';
			foreach ( $options as $id => $slug ) {
				$html .= "<option value='{$id}'>{$slug}</option>";
			}
			$html .= '</select>';

			$html .= '<div class="cpo-settings-btns-wrapper">';
			$html .= '<button id="js_cpo_export" type="button" class="button-primary cpo-btn-export">' . esc_html__( 'Export', 'uni-cpo' ) . '</button>';
			$html .= '<button id="js_cpo_view" type="button" class="button-primary cpo-btn-view">' . esc_html__( 'View', 'uni-cpo' ) . '</button>';
			$html .= '<div class="cpo-settings-btns-wrapper-import">';
			$html .= '<input id="js_cpo_import_file" type="file" name="cpo_import"/>';
			$html .= '<button id="js_cpo_import" type="button" class="button-primary">' . esc_html__( 'Import', 'uni-cpo' ) . '</button>';
			$html .= '</div>';
			$html .= '</div>';

			$html .= '<div id="js_cpo_table"></div>';
		} else {
			$html .= '<p><strong>' . esc_html__( 'This functionality is included in PRO version of the plugin', 'uni-cpo' ) . '</strong></p>';
		}
