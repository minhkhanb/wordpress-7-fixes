    ...
    */
	...
	public static function template( $data, $post_data = array() ) {
		...
		$suboptions           = ( isset( $data['settings']['cpo_suboptions']['data']['cpo_radio_options'] ) )
			...
			: array();
		$cpo_general_main     = $data['settings']['cpo_general']['main'];
		$cpo_mode_radio       = ( isset( $cpo_general_main['cpo_mode_radio'] ) )
            ? $cpo_general_main['cpo_mode_radio']
            : 'classic';
		$cpo_change_image     = ( isset( $cpo_general_main['cpo_is_changeimage'] ) )
			? $cpo_general_main['cpo_is_changeimage']
			: 'no';
		$cpo_is_imagify     = ( isset( $cpo_general_main['cpo_is_imagify'] ) )
			? $cpo_general_main['cpo_is_imagify']
			: 'no';
        $cpo_encoded_image     = ( isset( $cpo_general_main['cpo_encoded_image'] ) )
            ? $cpo_general_main['cpo_encoded_image']
            : '';
		$cpo_general_advanced = $data['settings']['cpo_general']['advanced'];
		$cpo_validation_main  = ( isset( $data['settings']['cpo_validation']['main'] ) )
			? $data['settings']['cpo_validation']['main']
        ...
        $cpo_is_resetbutton = ( isset( $cpo_general_main['cpo_is_resetbutton'] ) )
            ? ( 'yes' === $cpo_general_main['cpo_is_resetbutton'] ) ? true : false
            : false;

        $cpo_resetbutton_text = ( isset( $cpo_general_main['cpo_resetbutton_text'] ) )
            ? $cpo_general_main['cpo_resetbutton_text']
            : 'Clear option';
    ...
    <div
		...
		foreach ( $suboptions as $suboption ) :
			...
			if ( unicpo_fs()->is__premium_only() ) {
				if ( ( 'yes' === $cpo_change_image || 'yes' === $cpo_is_imagify ) && ( ! empty( $suboption['attach_id'] ) || ! empty( $suboption['attach_id_r'] ) ) ) {
					...
					$image_thumb_single    = wp_get_attachment_image_src( $replacement_attach_id, 'woocommerce_single' );
					$data_image            = $image_thumb_single[0];
			...
			} else if ( ! $selected_chosen && empty( $default_value ) && $suboption['def'] === 'checked' ) {
				$attributes_new['checked'] = 'checked';
				$selected_chosen = true;
			}
			?>
            ...
            <label
                ...
                switch ( $cpo_mode_radio ) {
					...
					<span class="uni-cpo-option-label__radio"
                          data-image="<?php echo esc_attr( $data_image ) ?>"></span>
	                <span class="uni-cpo-option-label__text"
	                      data-image="<?php echo esc_attr( $data_image ) ?>">
                    ...
                    case 'colour' :
	                    if ( unicpo_fs()->is__premium_only() ) {
		                    ...
		                    <?php if ( ! empty( $suboption['suboption_colour'] ) ) { ?>
                                <span class="uni-cpo-option-label__colour-wrap"
                                      data-hex="<?php echo esc_attr( $suboption['suboption_colour'] ) ?>"
                                      data-image="<?php echo esc_attr( $data_image ) ?>"
                                      data-tip="<?php esc_html_e( uni_cpo_get_proper_option_label_sp( $suboption['label'] ) ); ?>">
                    		        <span class="uni-cpo-option-label__colour"></span>
                    	        </span>
                                <?php if ( ! empty( $suboption['suboption_text'] ) ) { ?>
                                    <span class="uni-cpo-option-label__description">
                    ...
                    case 'image' :
						...
						<?php } else { ?>
                            <span class="uni-cpo-option-label__empty">
	                    		<?php esc_html_e( 'Please add image to suboption', 'uni-cpo' ) ?>
	                    	</span>
						<?php } ?>
					        </span>
                        <?php if ( ! empty( $suboption['suboption_text'] ) ) { ?>
		...
		<?php endforeach; ?>
        <?php if ( $cpo_is_resetbutton ) { ?>
            <div class = "uni-cpo-radio-resetbutton uni-clear">
                <button class = "uni_cpo_<?php echo esc_attr( $slug ); ?>-field-reset-button js-uni-cpo-field-<?php echo esc_attr( $type ); ?>-reset-button"><?php esc_html_e( $cpo_resetbutton_text ); ?></button>
            </div>
        <?php } ?>
	...
	public static function get_css( $data ) {
		...
		$margin               = ( isset( $data['settings']['advanced']['layout'] ) && isset( $data['settings']['advanced']['layout']['margin'] ) )
			? $data['settings']['advanced']['layout']['margin']
			: array();
		$padding              = ( isset( $data['settings']['style']['text_mode_item'] ) && isset( $data['settings']['style']['text_mode_item']['padding'] ) )
			? $data['settings']['style']['text_mode_item']['padding']
			: array();
		$cpo_general_main     = $data['settings']['cpo_general']['main'];
		...
		?>
			...
			<?php } else if ( $cpo_mode_radio === 'colour' ) { ?>
				.uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-radio-option-label {
            	...
            	.uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-option-label__colour-wrap {
	            	...
	            	<?php if ( $cpo_geom_radio === 'circle' ) { ?> border-radius: 100%; <?php } else { ?> border-radius: 0%; <?php } ?>
	            }
			...
			<?php } else if ( $cpo_mode_radio === 'image' ) { ?>
				.uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-radio-option-label {
                    <?php if ( ! empty( $main['width_px'] ) ) { ?> width: <?php echo esc_attr( $correct_width ); ?>px; <?php } ?>
            		<?php if ( ! empty( $border['gap_px'] ) ) { ?> margin-right: <?php echo esc_attr( $border['gap_px'] ); ?>px; margin-bottom: <?php echo esc_attr( $border['gap_px'] ); ?>px;<?php } ?>
            	}
			...
			<?php } else if ( $cpo_mode_radio === 'text' ) { ?>
	        	.uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-radio-option-label {
            		<?php if ( ! empty( $border['gap_px'] ) ) { ?> margin-right: <?php echo esc_attr( $border['gap_px'] ); ?>px; margin-bottom: <?php echo esc_attr( $border['gap_px'] ); ?>px;<?php } ?>
            	}
