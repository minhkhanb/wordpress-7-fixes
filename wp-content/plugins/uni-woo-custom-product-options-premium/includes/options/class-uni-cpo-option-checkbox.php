...
/*
*   Uni_Cpo_Option_Checkbox class
*
*/

class Uni_Cpo_Option_Checkbox extends Uni_Cpo_Option implements Uni_Cpo_Option_Interface {
	...
	public static function get_title() {
		return __( 'Checkbox Inputs', 'uni-cpo' );
	}
    ...
    */
	...
	public function get_edit_field( $data, $value, $context = 'cart' ) {
        ...
        <div class="cpo-cart-item-option-wrapper uni-node-<?php echo esc_attr($id) ?> <?php if ( 'order' === $context ) { echo esc_attr( 'uni-admin-order-item-option-wrapper' ); } ?>">
			...
			<div class="uni-admin-order-item-option-checkboxes">
        		<?php $value_str = '';
		        if ( ! empty( $value ) ) {
		            if ( is_array( $value ) ) {
				...
				foreach ( $suboptions as $suboption ) :
					...
					if ( ! empty( $value ) && in_array( $suboption['slug'], $value ) ) {
						$attributes_new['checked'] = 'checked';
					} elseif ( isset( $suboption['def'] ) && ! empty( $suboption['def'] ) ) {
						$attributes_new['checked'] = 'checked';
					}
				    ...
				    ?>
		                <span>
		                    <input
		                            ...
		                            <?php echo self::get_custom_attribute_html( $attributes_new ); ?> />
		                    <label><?php esc_html_e( uni_cpo_get_proper_option_label_sp( $suboption['label'] ) ); ?></label>
		                </span>
					<?php
				    } else {
				    ?>
				...
				endforeach;
		        if ( 'cart' === $context && !$is_cart_edit ) {
			        ?>
		            <input
		                    class="cpo-cart-item-option cpo-cart-item-option-multi"
		                    name="<?php echo esc_attr( $slug ) ?>"
			...
			'settings' => array(
				...
				'cpo_general'     => array(
					'main'     => array(
						'cpo_slug'        => '',
						'cpo_is_required' => 'no',
                        'cpo_mode_checkbox' => 'classic',
                        'cpo_geom_checkbox' => 'circle',
	...
	public static function js_template() {
        ...
        <script id="js-builderius-module-<?php echo self::get_type(); ?>-tmpl" type="text/template">
            ...
            {{ const cpo_mode_checkbox = uniGet(cpo_general.main, 'cpo_mode_checkbox', 'classic'); }}
            {{ const cpo_geom_checkbox = uniGet(cpo_general.main, 'cpo_geom_checkbox', 'circle'); }}

            {{ let checkboxOptions = []; }}
            {{ if (typeof cpo_suboptions.data !== 'undefined') { }}
            {{ checkboxOptions = Object.values(uniGet(cpo_suboptions.data, 'cpo_radio_options', [])); }}
            {{ } }}
            ...
            <div
                    id="{{- id_name }}"
                    class="uni-module uni-module-{{- type }} uni-module-{{- type }}-{{- cpo_mode_checkbox }}-mode uni-node-{{- id }} {{- class_name }} {{- emptyModule }}"
                    data-node="{{- id }}"
                    data-type="{{- type }}">
            <style>
            	.uni-node-{{= id }} {
	            ...
	            {{ } else if ( cpo_mode_checkbox === 'colour' ) { }}
	            	.uni-node-{{= id }} .uni-cpo-checkbox-option-label {
	            	...
	            	.uni-node-{{= id }} .uni-cpo-option-label__colour-wrap {
		            	...
		            	{{ if ( cpo_geom_checkbox === 'circle' ) { }} border-radius: 100%; {{ } else { }} border-radius: 0%; {{ } }}
		            }
		            {{ if (checkboxOptions) { }}
	            ...
	            {{ } else if ( cpo_mode_checkbox === 'image' ) { }}
	            	.uni-node-{{= id }} .uni-cpo-checkbox-option-label {
                        {{ if ( width_px ) { }} width: {{= correct_width }}px; {{ } }}
	            		{{ if ( gap_px ) { }} margin-right: {{= gap_px }}px; margin-bottom: {{= gap_px }}px; {{ } }}
	            	}
	            ...
	            {{ } else if ( cpo_mode_checkbox === 'text' ) { }}
	            	.uni-node-{{= id }} .uni-cpo-checkbox-option-label {
	            		{{ if ( gap_px ) { }} margin-right: {{= gap_px }}px; margin-bottom: {{= gap_px }}px; {{ } }}
	            	}
            ...
            {{ if ( checkboxOptions ) { }}
                {{ for (let i = 0; i < checkboxOptions.length; i++) { }}
	                ...
	                <input
	                    ...
	                    {{ if ( typeof option.def !== 'undefined' && option.def.length > 0 ) { }} checked="checked" {{ } }}
	                    id="uni_cpo_{{- cpo_slug }}-field-{{- option.slug }}"
	                    name="{{- cpo_slug }}"
	                    type="checkbox"
	                    value="{{- option.slug }}"/>
	                <label for="uni_cpo_{{- cpo_slug }}-field-{{- option.slug }}" class="uni-cpo-option-label uni-cpo-checkbox-option-label uni-node-{{= id }}-cpo-option-label-{{- option.slug }} {{- option.suboption_class }}">
	                	{{ if ( cpo_mode_checkbox === 'classic' ) { }}
		                ...
		                {{ } else if ( cpo_mode_checkbox === 'text' ) { }}
		                	<span class="uni-cpo-option-label__text-content">
		                        {{- option.label }}
		                    </span>
		                {{ } }}
	                </label>
	...
	public static function template( $data, $post_data = array() ) {
		...
		$suboptions           = ( isset( $data['settings']['cpo_suboptions']['data']['cpo_radio_options'] ) )
			...
			: array();
		$cpo_general_main     = $data['settings']['cpo_general']['main'];
		$cpo_mode_checkbox       = ( isset( $cpo_general_main['cpo_mode_checkbox'] ) )
            ? $cpo_general_main['cpo_mode_checkbox']
            : 'classic';
		$cpo_change_image     = ( isset( $cpo_general_main['cpo_is_changeimage'] ) )
			? $cpo_general_main['cpo_is_changeimage']
			: 'no';
		$cpo_general_advanced = $data['settings']['cpo_general']['advanced'];
		$cpo_validation_main  = ( isset( $data['settings']['cpo_validation']['main'] ) )
			? $data['settings']['cpo_validation']['main']
		...
		$css_class         = array(
			...
			'uni-module-' . $type,
			'uni-node-' . $id,
			'uni-module-' . $type . '-' . $cpo_mode_checkbox . '-mode'
		);
		$input_css_class[] = $slug . '-field';
		...
		$suboption_classes_default = array( 'uni-cpo-option-label', 'uni-cpo-checkbox-option-label' );
		?>
    <div
            id="<?php echo implode( ' ', array_map( function ( $el ) {
				return esc_attr( $el );
			}, $css_id ) ); ?>"
			<?php if ( count( $suboptions ) === 0 ) { ?>
				data-tip="<?php esc_attr_e('Please add suboptions to this option!', 'uni-cpo') ?>"
			<?php } ?>
            class="<?php echo implode( ' ', array_map( function ( $el ) {
				return esc_attr( $el );
			}, $css_class ) ); ?>"
		...
		foreach ( $suboptions as $suboption ) :
			...
			$attributes_new = $attributes;
			if ( 'yes' === $cpo_change_image && ( ! empty( $suboption['attach_id'] ) || ! empty( $suboption['attach_id_r'] ) ) ) {
				...
				$for_image_replace = array(
                    'data-img-id' => $replacement_attach_id,
					'data-img-full-uri' => $src_full,
                    'data-img-title' => uni_cpo_get_proper_option_label_sp( $suboption['label'] ),
                    'data-img-alt' => uni_cpo_get_proper_option_label_sp( $suboption['label'] ),
                    'data-img-uri' => $src,
                    'data-img-srcset' => $srcset,
                    'data-img-sizes' => $sizes,
					'data-img-src' => $src,
					'data-img-large_image' => $src_full,
					'data-img-large_image_width' => $width_full,
                    'data-img-large_image_height' => $height_full,
					'data-img-thumb-uri'          => $image_thumb[0],
					'data-img-width'              => $width,
					'data-img-height'             => $height
				);
				$attributes_new = $attributes + $for_image_replace;
		    }
			...
			if ( ! empty( $default_value ) && in_array( $suboption['slug'], $default_value ) ) {
				$attributes_new['checked'] = 'checked';
			} elseif ( empty( $default_value ) && isset( $suboption['def'] ) && ! empty( $suboption['def'] ) ) {
				$attributes_new['checked'] = 'checked';
			}
			?>
            <input
                    ...
                    id="<?php echo esc_attr( $slug ); ?>-field-<?php echo esc_attr( $suboption['slug'] ); ?>"
                    name="<?php echo esc_attr( $slug ); ?>[]"
                    type="checkbox"
                    value="<?php echo esc_attr( $suboption['slug'] ); ?>"
				    <?php echo self::get_custom_attribute_html( $attributes_new ); ?> />
            <label
                    for="<?php echo esc_attr( $slug ); ?>-field-<?php echo esc_attr( $suboption['slug'] ); ?>"
                    class="<?php echo implode( ' ', array_map( function ( $el ) {
						return esc_attr( $el );
					}, $suboption_classes ) ); ?>">
				<?php
                switch ( $cpo_mode_checkbox ) {
                    case 'classic' :
                        ?>
					<span class="uni-cpo-option-label__checkbox"></span>
	                <span class="uni-cpo-option-label__text">
                        <?php esc_html_e( uni_cpo_get_proper_option_label_sp( $suboption['label'] ) ); ?>
                        <?php if ( ! empty( $suboption['suboption_text'] ) ) { ?>
                            <span class="uni-cpo-option-label__description">
                                <?php esc_html_e( $suboption['suboption_text'] ); ?>
                            </span>
                        <?php } ?>
                    </span>
				<?php
                    ...
                    case 'colour' :
                        ?>
					<?php if ( ! empty( $suboption['suboption_colour'] ) ) { ?>
						<span class="uni-cpo-option-label__colour-wrap" data-tip="<?php esc_attr_e( uni_cpo_get_proper_option_label_sp( $suboption['label'] ) ); ?>">
                    		<span class="uni-cpo-option-label__colour"></span>
                    	</span>
                        <?php if ( ! empty( $suboption['suboption_text'] ) ) { ?>
                            <span class="uni-cpo-option-label__description">
				...
				<?php
                    ...
                    case 'image' :
                        ?>
					<span class="uni-cpo-option-label__image-wrap" data-tip="<?php esc_attr_e( uni_cpo_get_proper_option_label_sp( $suboption['label'] ) ); ?>">
						<?php if ( ! empty( $suboption['attach_uri'] ) ) { ?>
		                    <img
		                            class="uni-cpo-option-label__image"
		                            src="<?php echo esc_url( $suboption['attach_uri'] ); ?>"
		                            alt="<?php esc_attr_e( uni_cpo_get_proper_option_label_sp( $suboption['label'] ) ); ?>"/>
						<?php } else { ?>
							<span class="uni-cpo-option-label__empty">
	                    		Please add image to suboption
	                    	</span>
						<?php } ?>
					</span>
                    <?php if ( ! empty( $suboption['suboption_text'] ) ) { ?>
				...
				<?php
                        break;
                    case 'text' : ?>
					<span class="uni-cpo-option-label__text-content"><?php esc_html_e( uni_cpo_get_proper_option_label_sp( $suboption['label'] ) ); ?></span>
				<?php
                        break;
                }
                ?>
	...
	public static function get_css( $data ) {
		...
		$bg                   = ( isset( $data['settings']['style']['background'] ) )
			? $data['settings']['style']['background']
			: array();
		$border               = $data['settings']['style']['border'];
		$margin               = $data['settings']['advanced']['layout']['margin'];
		$padding              = $data['settings']['style']['text_mode_item']['padding'];
		$cpo_general_main     = $data['settings']['cpo_general']['main'];
		$cpo_mode_checkbox    = $cpo_general_main['cpo_mode_checkbox'];
		$cpo_geom_checkbox    = $cpo_general_main['cpo_geom_checkbox'];
		$cpo_general_advanced = $data['settings']['cpo_general']['advanced'];
		...
		?>
            ...
            <?php } ?>
			<?php if ( $cpo_mode_checkbox === 'classic' ) { ?>
				.uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-option-label__text {
					<?php if ( ! empty( $font['color'] ) ) { ?> color: <?php echo esc_attr( $font['color'] ); ?>;<?php } ?>
			...
			<?php } else if ( $cpo_mode_checkbox === 'colour' ) { ?>
				.uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-checkbox-option-label {
                    <?php if ( ! empty( $main['width_px'] ) ) { ?> width: <?php echo esc_attr( $correct_width ); ?>px; <?php } ?>
            		<?php if ( ! empty( $border['gap_px'] ) ) { ?> margin-right: <?php echo esc_attr( $border['gap_px'] ); ?>px; margin-bottom: <?php echo esc_attr( $border['gap_px'] ); ?>px; <?php } ?>
            	}
            	.uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-option-label__colour-wrap {
            		<?php if ( ! empty( $border['offset_px'] ) ) { ?> padding: <?php echo esc_attr( $border['offset_px'] ); ?>px;<?php } ?>
                    <?php if ( ! empty( $border['width_px'] ) ) { ?> border-width: <?php echo esc_attr( $border['width_px'] ); ?>px!important; border-style: solid; <?php } else { ?> border-width: 0!important; border-style: solid; <?php } ?>
	            	<?php if ( $cpo_geom_checkbox === 'circle' ) { ?> border-radius: 100%; <?php } else { ?> border-radius: 0%; <?php } ?>
	            }
	            <?php foreach ( $suboptions as $suboption ) : ?>
					.uni-node-<?php echo esc_attr( $id ); ?> input:checked + label.uni-node-<?php echo esc_attr( $id ); ?>-cpo-option-label-<?php echo esc_attr( $suboption['slug'] ); ?> .uni-cpo-option-label__colour-wrap {
    					border-color: <?php echo esc_attr( $suboption['suboption_colour'] ); ?>!important;
    				}
    				.uni-node-<?php echo esc_attr( $id ); ?> label.uni-node-<?php echo esc_attr( $id ); ?>-cpo-option-label-<?php echo esc_attr( $suboption['slug'] ); ?> .uni-cpo-option-label__colour-wrap .uni-cpo-option-label__colour {
    					background-color: <?php echo esc_attr( $suboption['suboption_colour'] ); ?>;
    				}
				<?php endforeach; ?>
				.uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-option-label__colour-wrap .uni-cpo-option-label__colour {
            		<?php if ( ! empty( $main['width_px'] ) ) { ?> width: <?php echo esc_attr( $main['width_px'] ); ?>px;height: <?php echo esc_attr( $main['width_px'] ); ?>px; <?php } ?>
            		<?php if ( $cpo_geom_checkbox === 'circle' ) { ?> border-radius: 100%; <?php } else { ?> border-radius: 0%; <?php } ?>
            	}
			<?php } else if ( $cpo_mode_checkbox === 'image' ) { ?>
				.uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-checkbox-option-label {
                    <?php if ( ! empty( $main['width_px'] ) ) { ?> width: <?php echo esc_attr( $correct_width ); ?>px; <?php } ?>
            		<?php if ( ! empty( $border['gap_px'] ) ) { ?> margin-right: <?php echo esc_attr( $border['gap_px'] ); ?>px; margin-bottom: <?php echo esc_attr( $border['gap_px'] ); ?>px;<?php } ?>
            	}
            	.uni-node-<?php echo esc_attr( $id ); ?> input:checked + label .uni-cpo-option-label__image-wrap {
            		<?php if ( ! empty( $border['color_active'] ) ) { ?> border-color: <?php echo esc_attr( $border['color_active'] ); ?>!important;<?php } ?>
            	}
            	.uni-node-<?php echo esc_attr( $id ); ?> label:hover .uni-cpo-option-label__image-wrap {
					<?php if ( ! empty( $border['color_hover'] ) ) { ?> border-color: <?php echo esc_attr( $border['color_hover'] ); ?>!important;<?php } ?>
            	}
				@media only screen and (max-width: 1024px) {
					.uni-node-<?php echo esc_attr( $id ); ?> label:hover .uni-cpo-option-label__image-wrap {
						<?php if ( ! empty( $border['color'] ) ) { ?> border-color: <?php echo esc_attr( $border['color'] ); ?>!important;<?php } ?>
					}
				}
            	.uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-option-label__image-wrap {
            		<?php if ( ! empty( $border['offset_px'] ) ) { ?> padding: <?php echo esc_attr( $border['offset_px'] ); ?>px;<?php } ?>
                    <?php if ( ! empty( $border['width_px'] ) ) { ?> border-width: <?php echo esc_attr( $border['width_px'] ); ?>px!important; border-style: solid; <?php } else { ?> border-width: 0!important; border-style: solid; <?php } ?>
            		<?php if ( ! empty( $border['color'] ) ) { ?> border-color: <?php echo esc_attr( $border['color'] ); ?>!important;<?php } ?>
	            	<?php if ( $cpo_geom_checkbox === 'circle' ) { ?> border-radius: 100%; <?php } else { ?> border-radius: 0%; <?php } ?>
	            }
            	.uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-option-label__image-wrap img {
            		<?php if ( ! empty( $main['width_px'] ) ) { ?> width: <?php echo esc_attr( $main['width_px'] ); ?>px;<?php } ?>
					<?php if ( !empty( $main['height_px'] ) && empty( $main['width_px'] ) ) { ?> width:auto;height: <?php echo esc_attr( $main['height_px'] ); ?>px;<?php } ?>
            		<?php if ( $cpo_geom_checkbox === 'circle' ) { ?> border-radius: 100%; <?php } else { ?> border-radius: 0%; <?php } ?>
            	}
			<?php } else if ( $cpo_mode_checkbox === 'text' ) { ?>
	        	.uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-checkbox-option-label {
            		<?php if ( ! empty( $border['gap_px'] ) ) { ?> margin-right: <?php echo esc_attr( $border['gap_px'] ); ?>px; margin-bottom: <?php echo esc_attr( $border['gap_px'] ); ?>px;<?php } ?>
            	}
            	.uni-node-<?php echo esc_attr( $id ); ?> input:checked + label .uni-cpo-option-label__text-content {
            		<?php if ( ! empty( $border['color_active'] ) ) { ?>border-color: <?php echo esc_attr( $border['color_active'] ); ?>!important;<?php } ?>
				    <?php if ( ! empty( $bg['color_active'] ) ) { ?>background-color: <?php echo esc_attr( $bg['color_active'] ); ?>!important;<?php } ?>
					<?php if ( ! empty( $font['color_active'] ) ) { ?>color: <?php echo esc_attr( $font['color_active'] ); ?>;<?php } ?>
            	}
            	.uni-node-<?php echo esc_attr( $id ); ?> label:hover .uni-cpo-option-label__text-content {
					<?php if ( ! empty( $border['color_hover'] ) ) { ?>border-color: <?php echo esc_attr( $border['color_hover'] ); ?>!important;<?php } ?>
				    <?php if ( ! empty( $bg['color_hover'] ) ) { ?>background-color: <?php echo esc_attr( $bg['color_hover'] ); ?>!important;<?php } ?>
					<?php if ( ! empty( $font['color_hover'] ) ) { ?>color: <?php echo esc_attr( $font['color_hover'] ); ?>;<?php } ?>
            	}
            	.uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-option-label__text-content {
	            	<?php if ( ! empty( $font['color'] ) ) { ?> color: <?php echo esc_attr( $font['color'] ); ?>;<?php } ?>
					<?php if ( ! empty( $font['text_align'] ) ) { ?> text-align: <?php echo esc_attr( $font['text_align'] ); ?>;<?php } ?>
					<?php if ( $font['font_family'] !== 'inherit' ) { ?> font-family: <?php echo esc_attr( $font['font_family'] ); ?>;<?php } ?>
					<?php if ( $font['font_style'] !== 'inherit' ) { ?> font-style: <?php echo esc_attr( $font['font_style'] ); ?>;<?php } ?>
					<?php if ( ! empty( $font['font_weight'] ) ) { ?> font-weight: <?php echo esc_attr( $font['font_weight'] ); ?>;<?php } ?>
					<?php if ( ! empty( $font['font_size']['value'] ) ) { ?> font-size: <?php echo esc_attr( "{$font['font_size']['value']}{$font['font_size']['unit']}" ) ?>; <?php } ?>
					<?php if ( ! empty( $font['letter_spacing'] ) ) { ?> letter-spacing: <?php echo esc_attr( $font['letter_spacing'] ); ?>em;<?php } ?>
                    <?php if ( ! empty( $main['width_px'] ) ) { ?> width: <?php echo esc_attr( $main['width_px'] ); ?>px; <?php } ?>
                    <?php if ( ! empty( $main['height_px'] ) ) { ?> height: <?php echo esc_attr( $main['height_px'] ); ?>px; line-height: <?php echo esc_attr( $main['height_px'] ); ?>px; <?php } else if ( $main['width_px'] ) { ?> height: <?php echo esc_attr( $main['width_px'] ); ?>px; line-height: <?php echo esc_attr( $main['width_px'] ); ?>px; <?php } ?>
                    <?php if ( ! empty( $border['width_px'] ) ) { ?> border-width: <?php echo esc_attr( $border['width_px'] ); ?>px!important; border-style: solid; <?php } else { ?> border-width: 0!important; border-style: solid; <?php } ?>
            		<?php if ( ! empty( $border['color'] ) ) { ?> border-color: <?php echo esc_attr( $border['color'] ); ?>!important;<?php } ?>
            		<?php if ( ! empty( $bg['color'] ) ) { ?> background-color: <?php echo esc_attr( $bg['color'] ); ?>!important;<?php } ?>
	            	<?php if ( $cpo_geom_checkbox === 'circle' ) { ?> border-radius: 100%; <?php } else { ?> border-radius: 0%; <?php } ?>
	        ...
	        <?php } ?>
            .uni-node-<?php echo esc_attr( $id ); ?> .uni-cpo-option-label__description {
                <?php if ( ! empty( $description['color'] ) ) { ?> color: <?php echo esc_attr( $description['color'] ); ?>!important;<?php } ?>
                <?php if ( ! empty( $description['font_weight'] ) ) { ?> font-weight: <?php echo esc_attr( $description['font_weight'] ); ?>!important;<?php } ?>
                <?php if ( ! empty( $description['font_size_desc']['value'] ) ) { ?> font-size: <?php echo esc_attr( "{$description['font_size_desc']['value']}{$description['font_size_desc']['unit']}" ) ?>!important; <?php } ?>
            }
	...
	public function calculate( $form_data ) {
		...
		$form_data[ $post_name ] = is_array( $form_data[ $post_name ] )
			? $form_data[ $post_name ]
			: explode( ',', $form_data[ $post_name ] );

		$form_data[ $post_name ] = array_map(
				function($item) {
					return trim($item);
				},
			$form_data[ $post_name ]
		...
		if ( ! empty( $form_data[ $post_name ] ) ) {
			...
			if ( isset( $suboptions['data']['cpo_radio_options'] ) && ! empty( $suboptions['data']['cpo_radio_options'] ) ) {
				foreach ( $suboptions['data']['cpo_radio_options'] as $k => $v ) {
					if ( is_array( $form_data[ $post_name ] ) && in_array( ( ( ! empty( $v['slug'] ) ) ? $v['slug'] : '' ), $form_data[ $post_name ] ) ) {
						$total_price += ( ! empty( $v['rate'] ) ) ? floatval( $v['rate'] ) : 0;
						$options_slugs[] = $v['slug'];
						$options_names[] = $v['label'];
					}
				}
                if ( ! empty( $options_slugs ) ) {
                    return array(
                        $post_name => array(
                            'calc'       => $total_price,
                            'cart_meta'  => $options_slugs,
                            'order_meta' => $options_names
                        ),
                        $post_name . '_count'        => array(
	                        'calc'       => count( $options_slugs ),
	                        'cart_meta'  => count( $options_slugs ),
	                        'order_meta' => count( $options_slugs )
                        ),
                    );
                } else {
	                return array(
