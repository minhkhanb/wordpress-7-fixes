...
/*
*   Uni_Cpo_Option_Distance_By_Postcode class
*
*/

class Uni_Cpo_Option_Distance_By_Postcode extends Uni_Cpo_Option implements Uni_Cpo_Option_Interface {
	...
	public static function get_type() {
		return 'distance_by_postcode';
	}

	public static function get_title() {
		return __( 'Distance By Postcode', 'uni-cpo' );
	}
    ...
    */
	...
	public static function js_template() {
        ...
        <script id="js-builderius-module-<?php echo self::get_type(); ?>-tmpl" type="text/template">
            ...
            {{ let cpo_postcode_type = data.settings.cpo_general.main.cpo_postcode_type; }}
        	...
        	{{ } }}
            {{ if ( cpo_postcode_type && cpo_postcode_type === 'dual' ) { }}
            <input
                    autocomplete = "off"
                    class="{{- cpo_slug }}-field js-uni-cpo-field-{{- type }}"
                    id="{{- cpo_slug }}-field-origin"
                    name="{{- cpo_slug }}_origin"
                    type="text"
                    value=""/>
            {{ } }}
            <input
	...
	public static function template( $data, $post_data = array() ) {
		...
		$option               = false;
		$postcode_mode           = $cpo_general_main['cpo_postcode_type'];
    ...
    <div
		...
		<?php } ?>
		<?php if ('dual' === $postcode_mode) { ?>
            <input
                    class="<?php echo implode( ' ', array_map( function ( $el ) {
						return esc_attr( $el );
					}, $input_css_class ) ); ?>"
                    id="<?php echo esc_attr( $slug ); ?>-field-origin"
                    name="<?php echo esc_attr( $slug ); ?>_origin"
                    type="text"
                    value="<?php echo esc_attr( $default_value ); ?>"
        ...
        <input
                ...
                name="<?php echo esc_attr( $slug ); ?>"
                data-parsley-wrongpost="true"
                type="text"
                value="<?php echo esc_attr( $default_value ); ?>"
			    <?php echo self::get_custom_attribute_html( $attributes ); ?> />
        <input
                type="hidden"
                id="js-uni-cpo-field-<?php echo esc_attr( $slug ); ?>-distance"
                name="<?php echo esc_attr( $slug ); ?>_distance" />
        </div>
		<?php
	...
	public function calculate( $form_data ) {
		...
		if ( ! empty( $form_data[ $post_name ] ) ) {
			...
			$cpo_general      = $this->get_cpo_general();
			$postcode_type    = $cpo_general['main']['cpo_postcode_type'];
			$origin_postcode  = ! empty( $cpo_general['main']['cpo_origin_postcode'] )
				? $cpo_general['main']['cpo_origin_postcode']
				: '';
			$postcode_country = $cpo_general['main']['cpo_postcode_country'];
			$api_key          = $plugin_settings['gmap_api_key'];
			$distance         = 0;
			$error            = '';
			...
			if ( empty( $postcode_country ) ) {
				if ( $postcode_type === 'dual' ) {
					$origin = $form_data[ $post_name . '_origin' ];
				} else {
					$origin = $origin_postcode;
				}
				$destination = $form_data[ $post_name ];
			} else {
				if ( $postcode_type === 'dual' ) {
					$origin = $form_data[ $post_name . '_origin' ] . ' ' . $postcode_country;
				} else {
					$origin = $origin_postcode . ' ' . $postcode_country;
				}
				$destination = $form_data[ $post_name ] . ' ' . $postcode_country;
			}

			$origin_encoded      = urlencode( $origin );
			$destination_encoded = urlencode( $destination );
			$url                 = "https://maps.googleapis.com/maps/api/distancematrix/json?origins={$origin_encoded}&destinations={$destination_encoded}&mode=driving&language=en&sensor=false&key={$api_key}";
			...
			if ( is_array( $response ) && ! is_wp_error( $response ) ) {
				...
				} elseif ( isset( $body['rows'][0]['elements'][0]['status'] )
				           && $body['rows'][0]['elements'][0]['status'] === 'NOT_FOUND' ) {
					$error = 'Nothing found. Probably, wrong post code.';
				}
			}
			...
			if ( ! empty( $price ) ) {
				return array(
					...
					$post_name . '_origin'   => array(
						'calc'       => 0,
						'cart_meta'  => $origin,
						'order_meta' => $origin
					),
					$post_name . '_distance' => array(
						'calc'       => !$error ? 0 : -1,
						'cart_meta'  => '',
						'order_meta' => ''
					)
				...
				return array(
					...
					$post_name . '_distance' => array(
						'calc'       => !$error ? floatval( $distance ) : -1,
						'cart_meta'  => $distance,
						'order_meta' => $distance
					)
				);
