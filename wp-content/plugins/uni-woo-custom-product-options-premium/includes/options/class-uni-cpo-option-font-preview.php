...
/*
*   Uni_Cpo_Option_Font_Preview class
*
*/

class Uni_Cpo_Option_Font_Preview extends Uni_Cpo_Option implements Uni_Cpo_Option_Interface {
	...
	public static function get_title() {
		return __( 'Font Preview', 'uni-cpo' );
	}
    ...
    */
	...
	public static function js_template() {
        ...
        <script id="js-builderius-module-<?php echo self::get_type(); ?>-tmpl" type="text/template">
            ...
            {{ const { cpo_slug, cpo_fontsampler_id } = data.settings.cpo_general.main; }}
            <div
                id="{{- id_name }}"
                ...
                {{ if ( cpo_fontsampler_id ) { }}
				<p>Chosen fonsampler ID #{{= cpo_fontsampler_id }}. Preview is not available.</p>
                {{ } }}
                {{ if ( !cpo_fontsampler_id ) { }}
                <p>Fonsampler ID is not chosen yet.</p>
                {{ } }}
            </div>
        </script>
		<?php
	...
	public static function template( $data, $post_data = array() ) {
		...
		$cpo_fontsampler_id   = absint($cpo_general_main['cpo_fontsampler_id']);
    	...
    	<div
			...
			<?php echo do_shortcode("[fontsampler id={$cpo_fontsampler_id}]") ?>
        </div>
		<?php
