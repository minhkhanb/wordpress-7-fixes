...
/*
*   Uni_Cpo_Option_Google_Map class
*
*/

class Uni_Cpo_Option_Google_Map extends Uni_Cpo_Option implements Uni_Cpo_Option_Interface {
    ...
    */
	...
	public static function js_template() {
        ...
        <script id="js-builderius-module-<?php echo self::get_type(); ?>-tmpl" type="text/template">
            ...
            {{ const padding = uniGet( data.settings.style, 'text_input.padding', {top:4,right:10,bottom:4,left:10,unit:'px'} ); }}
            {{ const { cpo_slug, cpo_is_required, cpo_type } = data.settings.cpo_general.main; }}
        	...
        	{{ } }}
	            <div
	                    class="{{- cpo_slug }}-field js-uni-cpo-field-{{- type }} uni-cpo-map-block"
	                    id="js-uni-cpo-field-{{- cpo_slug }}-map">
                    <span><?php esc_html_e('Google map is unavailable in the builder mode', 'uni-cpo') ?></span>
	            </div>
                <input class="uni-cpo-map-option-input" type="text"/>
            </div>
        </script>
		<?php
	}
	...
	public static function template( $data, $post_data = array() ) {
        ...
        $input_css_class[] = 'uni-cpo-map-option-input';
		...
		$map_center = ( isset( $cpo_general_main['cpo_map_center'] ) ) ? $cpo_general_main['cpo_map_center'] : '51.509865,-0.118092';
		$map_zoom = ( isset( $cpo_general_main['cpo_map_zoom'] ) ) ? $cpo_general_main['cpo_map_zoom'] : 11;

		/*$default_value = ( ! empty( $post_data ) && ! empty( $slug ) && ! empty( $post_data[$slug] ) )
			? $post_data[$slug]
			: $cpo_general_main['cpo_def_coord'];*/
		?>
    <div
		...
		<?php } ?>
			<div
					id="js-uni-cpo-field-<?php echo esc_attr( $slug ); ?>-map"
					class="js-uni-gmap-container"
					data-slug="<?php echo esc_attr( $slug ) ?>"
					data-coords="<?php echo esc_attr( $map_center ) ?>"
					data-zoom="<?php echo esc_attr( $map_zoom ) ?>"></div>
			<input
					type="hidden"
					id="js-uni-cpo-field-<?php echo esc_attr( $slug ); ?>-latlng"
					name="<?php echo esc_attr( $slug ); ?>_latlng" />
			<input
					type="text"
					class="<?php echo implode( ' ', array_map( function ( $el ) {
						return esc_attr( $el );
					}, $input_css_class ) ); ?>"
					id="js-uni-cpo-field-<?php echo esc_attr( $slug ); ?>"
	...
	public static function get_css( $data ) {
		...
		$padding       = ( ! empty( $data['settings']['style']['text_input']['padding'] ) ) ? $data['settings']['style']['text_input']['padding'] : '';
        ...
        <?php } ?>
        .uni-node-<?php echo esc_attr( $id ); ?> > div.js-uni-gmap-container {
        ...
        .uni-node-<?php echo esc_attr( $id ); ?> input[type="text"] {
	...
	public function calculate( $form_data ) {
		...
		if ( ! empty( $form_data[ $post_name ] ) ) {
			...
			if ( ! empty( $price ) ) {
				...
				return array(
					$post_name                   => array(
						'calc'       => $form_data[ $post_name ],
						'cart_meta'  => $form_data[ $post_name ],
						'order_meta' => $form_data[ $post_name ]
					),
					$post_name . '_latlng'                 => array(
						'calc'       => 0,
						'cart_meta'  => $form_data[ $post_name ],
						'order_meta' => $form_data[ $post_name ]
					)
